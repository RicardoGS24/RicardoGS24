...
class BST {
    ...
    public:
        ...
        int findNumRepsIP(T ip);
        int findNumIPs(TNode<T>* aux, T ip);
        void print();
        bool isEmpty();
...
template<class T>
int BST<T>::findNumRepsIP(T ip){
    return findNumIPs(root, ip);
}

template<class T>
int BST<T>::findNumIPs(TNode<T>* aux, T ip){
    int contador = 0;
    ...
    if (aux != nullptr){
        ...
        contador += findNumIPs(aux->left, ip);
        contador += findNumIPs(aux->right, ip);
    }
    return contador;
}
